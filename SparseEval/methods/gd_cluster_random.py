...
def generate_anchors_mlp_high(vectors, Y_train, num_anchors, gpu_id, num_refinement_steps=10, partial_epochs=2000, learning_rate=5e-4, **kwargs):
    return random.sample(range(vectors.shape[0]), num_anchors)
    """
    ...
    scenario = sys.argv[1]
    init_strategy_list = {'mmlu': 'random', 'winogrande': 'random'}
...
def main():
    ...
    root = f"result_data/main_result/{scenario}/random/{num_anchors}"
