...
def load_base_data(scenario):
    ...
    pt_data = torch.load(pt_path, map_location=torch.device('cpu'), weights_only=False)
    matrix = pt_data['matrix']
    models = pt_data['models']
    Y = matrix.numpy()
...
def train_proxy_model(X_train, y_train, num_anchors, learning_rate, partial_epochs, device):
    ...
    model = nn.Sequential(
        nn.Linear(num_anchors, 128), nn.GELU(), nn.Dropout(0.1),
...
def gradient_descent_mlp_evaluation(Y_train, Y_valid, Y_test, anchor_points, learning_rate, num_epochs, patience, gpu_id):
    ...
    # --- 2. 定义MLP模型 ---
    model = nn.Sequential(
        nn.Linear(len(anchor_points), 128), nn.GELU(), nn.Dropout(0.1),
        nn.Linear(128, 64), nn.GELU(), nn.Dropout(0.1),
        nn.Linear(64, 32), nn.GELU(), nn.Dropout(0.1),
        nn.Linear(32, 1), nn.Sigmoid()
    ).to(device)
        ...
        if epoch % 500 == 0:
            ...
            with torch.no_grad():
                ...
                if valid_loss <= best_valid_loss and valid_tau >= best_valid_tau:
                    ...
                    # print(torch.abs(pred_test - y_test).cpu().tolist())
...
def main():
    ...
    root = f"result_data/main_result/{scenario}/sparse_eval/{num_anchors}"
    ...
    while len(dump_results) < 10:
        try:
            ...
            if not held_out:
                ...
                llm_dataset_list = [
                    'arc', 'gsm8k', 'hellaswag', 'mmlu', 'truthfulqa', 'winogrande', 'extend_arc', 'gpqa'
                ]
                if scenario in llm_dataset_list:
                    cnt = 200
                else:
                    exit("未知场景，请检查场景名称。")
